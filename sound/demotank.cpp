...
void welcomeScreen()
{
    ...
    welcome_text += "5. Press Q to quit the game.\n";
...
void keyboard(unsigned char key, int x, int y)
{
    ...
    if(game)
    {
        ...
        else if(key == ' ' && !pause)
        {
                ...
                sndPlaySound(TEXT("explode.wav"), SND_ASYNC);

                sndPlaySound(TEXT("boom.wav"), SND_ASYNC |SND_LOOP);
...
void drawPlaneLeft(int x, int y)
{
   glColor3ub(35, 108, 23);
...
int main(int argc, char** argv)
{
    ...
    sndPlaySound(TEXT("explode.wav"),SND_ASYNC |SND_LOOP );
